#' Check for Unique IDs in a Fisheries VMS Dataset
#'
#' @description
...
#'
#' @param data A data frame or tibble containing VMS data with an ID column to be checked for uniqueness.
#'
#' @return If duplicate IDs are found, returns a tibble with duplicate IDs and their count,
...
#'
#' @details
#' The function uses the `dplyr` package for data manipulation. It first counts the total
...
check_unique_ids <- function(data) {
  ...
  cat("Duplicate IDs:", total_rows - unique_ids, "\n")
  
  if (total_rows != unique_ids) {
    # Return the duplicate IDs and their count
    duplicates <- data %>%
      group_by(ID) %>%
      filter(n() > 1) %>%
      summarise(count = n()) %>%
